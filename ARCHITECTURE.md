...
- 定位：从需求到PRD的全流程生产线，支持三种模式
  - **自主模式**（默认）：10章完整PRD，5-9轮对话（约30-60分钟）
  - **交互模式**：10章完整PRD，18-33轮对话（约2-4小时）
  - **轻量模式**：7章精简PRD，2-5轮对话（约10-15分钟）
- 触发时机：每次有新需求需要产出PRD时
- 输入：用户需求描述 + 知识库文件（如有）+ 已有PRD草稿（可选导入）
- 产出：完整/精简PRD文档 + 流程图 + 自检报告 + 知识发现清单（轻量模式仅PRD + 可选图）
...
│   │   ├── knowledge-discoveries.md  # 过程中发现的新业务知识（全模式）
│   │   └── diagrams/
...
下一阶段启动时，自动读取上一阶段的产出文件作为输入。每个阶段开始前对上游文件执行基本格式校验（front matter 必填字段、章节标题存在性），避免格式损坏导致静默失败。
...
5. **增量积累**：每次PRD过程中发现的新业务知识自动记录到 `knowledge-discoveries.md`，交付时可一键导入知识库
6. **跨需求一致性**：启动新需求时扫描 `requirements/` 下已有 PRD，提取关键规则和接口定义作为约束输入
