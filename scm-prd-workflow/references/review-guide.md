...
扫描 `requirements/` 下所有已交付 PRD 的 `review-report.md`，统计 CK 检查通过率趋势。仅在 `requirements/` 下有 ≥3 个已完成 PRD 时触发。

输出格式（Markdown 表格，直接展示在终端）：
...
|--------|--------|-----------|------|
| CK-0 需求回溯 | 100% | — | — |
| CK-3 业务规则 | 67% | CK-3.4 边界条件覆盖不足 | 撰写时重点关注边界值 |
| CK-5 可测试性 | 50% | CK-5.4 模糊词残留 | 加强模糊词自动修正 |
```

**用途**：帮助 AI 识别系统性弱项，在后续 PRD 撰写时主动加强对应方面。趋势结果不持久化为文件，仅在用户选择时展示。
