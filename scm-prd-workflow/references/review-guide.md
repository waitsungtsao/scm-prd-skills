...
**进度播报**：进入本阶段时输出 `▰▰▰▰▰▱ [5/{total}] 自检审查`。多轮审阅迭代期间步骤编号不变。
...
**核心原则：脚本做结构性/确定性检查，AI 做语义/叙事检查。两者不重叠。**

1. **脚本先行**：运行 `check-prd-consistency.py`，结果直接写入 review-report.md。脚本覆盖以下检查项（AI 不再重复执行）：
   - CK-1.8（章节编号连续）、CK-2.6a（ID 交叉引用完整性）
   - CK-5.4~5.9（模糊用语、同义反复、填充句、无信息修饰词）
   - 变更点覆盖检查、叙事信号检查
2. **AI 语义自检**：重新读取完整 PRD，执行以下语义检查（脚本无法做的判断）
3. 每项检查记录结果：通过/关键/一般/警告
4. 合并脚本结果和 AI 结果，生成 review-report.md
...
| CK-1.8 | 章节编号是否从第1章连续递增，无跳号（裁剪章节后编号应重排）**[脚本覆盖]** | 警告 |
...
| CK-2.6a | 交叉引用完整性（分层检查）：**必查** — G-XX 和 F-XXX 是否在定义位置和引用位置成对出现；**按需** — C-XX（update/mixed 类型时）和 IF-XXX（有接口时）是否成对出现。规则和异常使用局部编号，不纳入全局交叉引用检查 **[脚本覆盖]** | 关键 |
...
| CK-5.4 | 全文搜索模糊词："大概""可能""一般""适当""合理""及时""等" **[脚本覆盖]** | 警告 |
| CK-5.5 | 操作步骤数是否合理：单个功能点超过10步时审视是否可简化 | 警告 |
| CK-5.6 | 是否存在同义反复（同一信息用不同措辞重复表述）**[脚本覆盖]** | 警告 |
| CK-5.7 | 是否存在填充句（"本章是核心""以下将详细描述"等不传递信息的句子）**[脚本覆盖]** | 警告 |
| CK-5.8 | 是否存在过度解释（规则已清晰时附加冗余的括号解释或举例）**[脚本覆盖]** | 警告 |
| CK-5.9 | 是否存在无信息修饰词（"进行""相关的""一定程度上""基本上""总体来说"）**[脚本覆盖]** | 警告 |
...
输出到 review-report.md `## CK-9 多角色审查`，包含：每角色一张表（#、检查项、结果、问题描述、建议）+ 多角色审查总结表（严重性 x 角色 x 合计）。
...
输出到 `review-report.md`，包含：front matter（type、requirement_id、stage、prd_version、review_date）、检查结果摘要表（CK-0~CK-9 + CK-PT 各类别 x 通过/关键/一般/警告）、关键问题（检查项、问题描述、所在位置、修改建议）、一般问题、警告项、通过项摘要。CK-PT 行仅在用户选中"原型演示（PT-01）"时出现。
...
3b. 如选中"用户故事拆分"→ 生成 `user-stories.md`（提取规则见下方"用户故事拆分"）
3c. 如选中"自检趋势"→ 输出跨 PRD CK 通过率趋势（见下方"自检趋势分析"）
4. 将所有产出文件呈现给用户
...
输出到 `delivery-brief.md`，包含：关键假设表（[待确认]标记，按影响范围降序）、跨系统依赖表（IF-xxx 接口）、实施风险表（CK 结果 + 高复杂度功能点）、AI 补充内容（[推断]和[待确认]标记汇总）。仅聚合 PRD 已有信息，不产生新内容；无内容的节省略。

#### 测试用例骨架提取

从 PRD Ch.9 验收标准 + Ch.6 功能规则自动生成 `test-cases.md`，为 QA 团队提供测试 checklist 起点。

输出到 `test-cases.md`，包含：功能测试（每个 F-XXX 的正向流程、规则验证、异常场景）、接口测试（每个 IF-XXX 的正常/超时/异常数据）、边界与异常（CK-4 未覆盖场景）、非功能测试（Ch.8 量化指标）。
...
读取 `requirements/_constraints-index.yaml`，生成 Mermaid `graph LR` 接口依赖图 `requirements/_prd-dependency.mermaid`。

**生成规则**：
- 每个 PRD 渲染为节点（名称+版本），按 `scope_in` 系统域分组为 subgraph
- 从 `interfaces` 提取接口关系画连线，当前 PRD 节点加粗高亮
- 约束索引不存在或仅含 1 个 PRD 时跳过

#### 用户故事拆分

从 PRD 的 F-XXX 功能点 + Ch.9 验收标准自动生成用户故事草稿，输出到 `user-stories.md`。可导入 Jira / Linear 等项目管理工具。
...
**复杂度**：{S/M/L}（基于步骤数和规则数估算）
```

**生成规则**：
- 每个 F-XXX 生成 1-N 个用户故事（一个功能点如果包含多个独立操作角色/场景，可拆分）
- 验收标准从 Ch.9 中关联到该 F-XXX 的条目提取
- 复杂度估算：步骤≤3且规则≤2 = S；步骤≤6或规则≤5 = M；其余 = L
- 跨功能的集成测试单独生成故事（标注为"集成验证"类型）

#### 自检趋势分析

扫描 `requirements/` 下所有已交付 PRD 的 `review-report.md`，统计 CK 通过率趋势。仅在 ≥3 个已完成 PRD 时触发。

输出 Markdown 表格（检查项 | 通过率 | 最常见问题 | 建议），直接展示在终端，不持久化为文件。用于识别系统性弱项，后续撰写时主动加强。
...
自主模式下 Stage B 自动执行自检，除上述 CK-0~CK-9 外，增加以下检查：
...
| 检查项 | 检查方法 | 严重性 |
|--------|---------|--------|
| CK-11.1 | 每个 `[待确认]` 项是否都包含"当前假设"说明 | 关键 |
| CK-11.2 | 所有 `[待确认]` 和关键 `[推断]` 是否汇总到第10章§10.3的假设总览表 | 关键 |
| CK-11.3 | 关键业务逻辑（业务规则、状态流转、权限控制）是否都至少有 `[推断]` 标记（如非用户明确确认） | 警告 |
| CK-11.4 | 引用知识库内容时是否标注了来源文件和章节 | 警告 |
| CK-11.5 | `[待确认]` 项是否标注了影响程度（高/中/低） | 警告 |
| CK-11.6 | 假设总览表中的章节引用是否与PRD实际位置一致 | 警告 |

### 自主模式自检报告格式

自主模式的 review-report.md 在 front matter 中增加 `mode: autonomous`，检查结果摘要表中包含 CK-0 和 CK-11 行：
