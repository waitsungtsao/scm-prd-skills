...
| CK-5.5 | 操作步骤数是否合理：单个功能点的操作步骤超过10步时，审视是否可简化；对比同类系统标准操作步骤数 | 警告 |
...
| 严重性 | 含义 | 处理要求 |
|--------|------|---------|
| **关键** | 业务逻辑错误、数据安全隐患、规则冲突等，直接影响系统正确性 | 必须修正后才能交付 |
...
所有修改完成后，重新执行自检。重复直到没有"关键"问题。

### 变更摘要输出

每轮修改后、呈现完整 PRD 之前，先输出一个变更摘要，帮助用户快速定位改动：
...
> 选项：
> - **逐条审阅**：逐条查看每个假设并确认
> - **挑选需改项**：默认全部确认，勾选需要修改的项（进入 multiSelect，用户勾选后仅对选中项进入修改流程）
> - **全部确认**：所有假设无异议，全部确认
