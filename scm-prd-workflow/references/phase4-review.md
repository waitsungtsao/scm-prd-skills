...
| 检查项 | 检查方法 | 严重性 |
|--------|---------|--------|
| CK-1.1 | PRD模板的章节是否都已填写。**注意**：当 `requirement_type = update\|mixed` 时，未涉及变更的方面可不生成对应章节（由PRD尾部统一说明），不视为缺失 | 一般 |
...
| CK-2.6a | 交叉引用完整性：所有 G-XXX、C-XXX、F-XXX、IF-XXX、BR-XXX 编号是否在定义位置和引用位置成对出现 | 关键 |
...
| 检查项 | 检查方法 | 严重性 |
|--------|---------|--------|
| CK-3.1 | 每条业务规则是否有明确的触发条件 | 关键 |
| CK-3.2 | 每条业务规则是否有明确的处理逻辑（不含模糊表述） | 关键 |
...
| CK-9.7 | [安全] 涉及用户数据/金额的操作是否有权限控制和操作审计 | 关键 |
| CK-9.8 | [安全] 接口是否有身份验证、防重放、防注入的安全要求 | 警告 |
| CK-9.9 | [安全] 敏感数据（手机号、身份证、银行卡）是否有脱敏/加密要求 | 警告 |
...
当用户同意时，使用 Claude Code Agent 工具生成4个并行子任务。

使用 `AskUserQuestion`（MR-01）：

> header: "审查模式"
> 问题: "CK-9 多角色审查支持两种执行方式："
> 选项：
> - **顺序审查（默认）**：依次从4个角色视角审查，速度稍慢但上下文连贯
> - **并行审查（Agent）**：使用子代理并行审查，速度快但消耗更多资源
...
**注意**：每条变更记录应为**语义级别**的变更描述（"BR-003 从X改为Y"），而非文本差异（"第15行修改了3个字"）。
