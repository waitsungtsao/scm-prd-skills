...
| 维度 | 自主模式（默认） | 轻量模式 |
|------|----------------|---------|
| 适用场景 | 从简单功能到复杂需求的全光谱 | 简单需求、常规改动、团队有共识 |
| 录入深度 | 自适应（1-8 轮，鉴别诊断式） | 0-1 轮 |
| PRD 章节 | 10 章 | 7 章 |
| 产出文件 | intake + clarification + PRD + review-report | 仅 PRD（+ 可选 Mermaid 图） |
| 标记体系 | `[推断]` + `[待确认]` 三级体系 | 仅 `[待确认]`（每项+2复杂度分） |
...
│   │   ├── autonomous-mode.md             #     自主模式（自适应深度录入+生成+审阅）
│   │   ├── writing-guide.md              #     撰写指引（NP检查+逐章指引）
│   │   ├── review-guide.md               #     自检清单（CK-0~CK-10）
