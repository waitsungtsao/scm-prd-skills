...
- **约束索引生命周期管理**：`_constraints-index.yaml` 新增三态生命周期（活跃/已归档/已废弃）、`domain_tags` 受控域标签、`superseded_by` 替代关系追踪。支持归档/废弃/恢复操作，不移动文件夹
- **域分组启动摘要**：约束索引 ≥5 条时，启动阶段按 `domain_tags` 分组展示已有 PRD 概览（如"OMS (8): order-split V1.2, ..."）
- **参考文档归档（sources/）**：用户提供的参考文档复制到 `REQ-{date}-{name}/sources/`，`intake.md` 标注来源路径。路径指引模式解决文件先于 REQ 文件夹的时序矛盾
- **修订入口生命周期选项**：修订入口新增"管理 PRD 生命周期"选项，支持归档/废弃/恢复

### Changed

- **跨 PRD 扫描过滤**：Stage A 约束扫描和修订模式影响分析均跳过 `status: 已废弃` 条目
- **修订入口列表过滤**：仅列出 `status: 活跃` 的 PRD，已归档和已废弃不出现
- **约束索引提取规则**：新交付默认 `status: 活跃`；`domain_tags` 从 `scope_in` 自动提取；修订交付保留生命周期字段不变
- **向后兼容**：旧索引缺少新字段时自动降级（`domain_tags` → `[]`，`superseded_by` → `null`，旧 `status` 值映射为 `活跃`）

## [2026.04.12] - 2026-04-07

### Changed

- **SKILL 瘦身**：人本门控重设计 + 标记检查 + 跨 PRD 前置扫描
...
- **叙事驱动 PRD 生产**（phase3-write.md）：新增"叙事规划"步骤——AI 在撰写前先完成产品经理思考（核心叙事、表达维度与详略、重点标注、模块间逻辑主线），用户确认后再动笔。从"按模板填"变为"讲故事"
- **三层 ID 体系**（phase3-write.md, check-prd-consistency.py）：简化交叉引用编号——G/F 始终使用，C/IF 按需使用，BR/E 降级为功能点内局部序号（不再全局编号），DL 改用标题引用。正文中用叙事逻辑连接模块，减少标号串联
- **章节指引咨询化**（phase3-write.md）：各章节撰写指引从规定性改为咨询式，每章标注"叙事角色/何时省略/详略判断"。Ch.6 新增叙述式和变更对比表两种替代格式；Ch.7/8/9 强调只写特有内容
- **PRD 大纲重构**（autonomous-mode.md）：自主模式大纲从"功能点+规则分段+接口清单"变为"核心叙事+表达维度与详略+重点标注+功能清单+图表策略+ID策略"
- **模板标注为参考菜单**（prd-template.md）：每章增加 HTML 注释标注叙事角色和省略条件，明确模板是参考而非必填框架
- **CK 检查适配**（phase4-review.md）：CK-1.1 改为对照叙事规划检查章节覆盖；CK-2.6a 改为分层检查（G/F 必查，C/IF 按需）；CK-3 去除 BR 全局编号依赖
- **一致性脚本适配**（check-prd-consistency.py）：移除 BR/DL 全局扫描，C 仅 update/mixed 时检查，变更覆盖检查仅 update/mixed 时执行
...
- **Word 输出策略**（phase3-write.md）：明确 python-docx 依赖，增加 pandoc 回退和"不阻断交付"原则
